...
--NOTE Тут техника, их точки по моему просто не существуют. Да и режим отключен
function cp.SpawnVehicle()
    for k, v in pairs(ReadDataMap("swo_carsct")) do
        ...
        if not contradiction then simfphys.SpawnVehicleSimple("sim_fphys_van", v2[1], v2[2]) end
    end
...
--NOTE А тут gred который тоже не используется. Да и наверное вообще ненужен
...
function cp.PointsThink() --обработка точек, сколько людей из каждой команды и процесс захвата
    ...
    for i, point in pairs(SpawnPointsList.controlpoint[3]) do
        ...
        for _, v2 in pairs(ents.FindInSphere(v[1], 256)) do
            if not v2:IsPlayer() or not v2:Alive() or v2.Otrub then continue end
...
function cp.RoundEndCheck()
    ...
    for i = 1, 2 do
        if cp.WinPoints[i] >= 1000 then EndRound(i) end
    end
...
function cp.PlayerInitialSpawn(ply)
    ply:SetTeam(math.random(2))
end

function cp.PlayerCanJoinTeam(ply, teamID)
    if teamID == 3 then
        ply:ChatPrint("Не чето нехочу пока")
        return false
    end
end
