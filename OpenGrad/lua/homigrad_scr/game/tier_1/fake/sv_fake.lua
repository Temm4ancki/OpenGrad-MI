...
function Faking(ply) -- функция падения
	...
	if not ply.fake then
		...
		if SERVER then ply:DrawWorldModel(false) end
		local veh
		...
		if IsValid(rag) then
			...
			if tracea.Hit then pos:Add(Vector(0, 0, 64) * tracea.Fraction) end
			local trace = {
				start = pos,
				endpos = pos + Vector(0, 0, 64),
				filter = {ply, rag}
...
hook.Add("Think", "FakedShoot", function()
...
function PlayerMeta:DropWeapon1(wep)
	...
	if wep.Base == "salat_base" or wep.Base == "koishi_sweps" then
...
concommand.Add("fake", function(ply)
	...
	if ply.LeftLeg == 0 and ply.RightLeg == 0 then
		ply:ChatPrint("Сломаны ноги, не могу встать.")
		return nil
	end
...
hook.Add("Player Collide", "homigrad-fake", function(ply, hitEnt, data)
	...
	and not ply.fake and not hitEnt:IsPlayerHolding() and hitEnt:GetVelocity():Length() > 150) then
		timer.Simple(0, function()
...
hook.Add("OnPlayerHitGround", "GovnoJopa", function(ply, a, b, speed)
	if speed > 200 then
		...
		if traceResult.Entity:IsPlayer() and not traceResult.Entity.fake then Faking(traceResult.Entity) end
	end
end)
...
hook.Add("Player Think", "FakeControl", function(ply, time) --управление в фейке
	...
	if IsValid(ply.bull) then ply.bull:SetPos(rag:GetPos()) end
	...
	if ply:Alive() then
		...
		if ply:KeyDown(IN_SPEED) and (RagdollOwner(rag) and not RagdollOwner(rag).Otrub) and not timer.Exists("StunTime" .. ply:EntIndex()) then
			local bone = rag:TranslateBoneToPhysBone(rag:LookupBone("ValveBiped.Bip01_L_Hand"))
			local phys = rag:GetPhysicsObjectNum(rag:TranslateBoneToPhysBone(rag:LookupBone("ValveBiped.Bip01_L_Hand")))
			if ply.Organs["artery"] == 0 and not TwoHandedOrNo[ply.curweapon] then
				local shadowparams = {
					secondstoarrive = 0.5,
					pos = head:GetPos(),
					angle = angs,
		...
		if ply:KeyDown(IN_WALK) and not RagdollOwner(rag).Otrub and not timer.Exists("StunTime" .. ply:EntIndex()) then
