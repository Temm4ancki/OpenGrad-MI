...
if SERVER then
	...
	function ENT:Think()
		...
		for key, obj in pairs(ents.FindInSphere(SelfPos, 300)) do
			if not (obj == self) and self:CanSee(obj) then
				...
				elseif self:ShouldDamage(obj) and (math.random(1, 3) == 1) and (self.NextDmg < Time) then
					...
					Dmg:SetDamage(math.random(2, 6) * JMod.Config.PoisonGasDamage)
		...
		Phys:SetVelocity(Phys:GetVelocity() * .6)
	...
	function ENT:PhysicsCollide(data, physobj)
		self:GetPhysicsObject():ApplyForceCenter(data.HitNormal * 150)
	end
...
elseif CLIENT then
	...
	function ENT:Initialize()
		self.Col = Color(math.random(230, 250), math.random(25, 35), 155)
		self.Visible = true
		self.Show = true
		self.siz = 1
