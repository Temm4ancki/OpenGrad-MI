...
function ENT:UpdateArmor(taker)
	local ply
	if IsValid(taker:GetNWEntity("Ragdoll")) then
		ply = taker:GetNWEntity("Ragdoll")
	else
		ply = taker
	end

	if self.taken and not ply.ChestArmor == self then return end
	self.taken = true
...
if ent:GetClass() == "armor" and ent.taken then
	return false end
end)