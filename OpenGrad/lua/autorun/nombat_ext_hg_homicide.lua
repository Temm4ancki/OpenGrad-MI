...
if CLIENT then
	function Nombat_Cl_Init(  )
		...
		if IsValid(pl) then
			if pl then
				...
				local Ambient_Time = {236,125} --$ song time (in seconds)
				pl.NOMBAT_Amb_Delay = CurTime() -- (DONT EDIT)

				local Combat_Time = {122,106} --$ song time (in seconds)
				pl.NOMBAT_Com_Delay = CurTime() -- (DONT EDIT)
				pl.NOMBAT_Com_Cool = CurTime() -- (DONT EDIT)

				local packName = "hg_homicide" --$ MAKE SURE THIS IS THE SAME AS THE FOLDER NAME HOLDING THE SOUNDS
	...
	hook.Add( "InitPostEntity", "Nombat_Cl_Init_hg_homicide", Nombat_Cl_Init ) --$ change the "Nombat_Cl_Init_GAMENAME" to "Nombat_Cl_Init_" and your game name.
