...
function SWEP:Reload()
	...
	if not self.EngineRunning then
...
function SWEP:HandleDoorBreaking(entity, damage)
	...
	local hitCount = entity:GetNWInt("DoorHitCount", 0)
	hitCount = hitCount + 1
	entity:SetNWInt("DoorHitCount", hitCount)

	if hitCount >= 6 then
		local currentChance = entity:GetNWInt("DoorBreakChance", 0)
		if currentChance == 0 then
			local baseDamage = math.max(damage, 3)
