...
function Tags:GetNameColor( ply )
    ...
    -- Use player color if available, otherwise use team color
    local playerColor = ply:GetPlayerColor()
    if playerColor and (playerColor.r > 0 or playerColor.g > 0 or playerColor.b > 0) then
        return Color( playerColor.r * 255, playerColor.g * 255, playerColor.b * 255 )
    end
...
local function CustomChat_AddCustomTags( ply, text, isTeam, isDead )
    ...
    -- Use FakeName if available, otherwise use regular nick
    local displayName = ply:GetNWString("FakeName", "")
    if displayName == "" then
        displayName = ply:Nick()
    end
    
    Insert( Tags:GetNameColor( ply ) )
    Insert( displayName )
    Insert( messageColor )
    Insert( ": " .. text )
...
function Tags:OpenEditor()
    ...
    buttonAddPlayer.DoClick = function()
        ...
        for _, ply in ipairs( player.GetHumans() ) do
            ...
            local item = menuPlayers:AddOption( displayName, AddSteamID )
