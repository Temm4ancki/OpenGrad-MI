...
function CustomChat:AddMessage( contents, channelId )
    ...
    if not self.frame.channels[channelId] then
        ...
        -- Use FakeName if available, otherwise use regular nick
        local displayName = dmSpeaker:GetNWString("FakeName", "")
        if displayName == "" then
            displayName = dmSpeaker:Nick()
        end
        
        local channel = self.frame:CreateChannel( channelId, displayName, dmSpeaker )
        channel.isDM = true
    end
