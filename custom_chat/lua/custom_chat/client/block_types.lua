...
blocks["player"] = function( value, ctx )
    ...
    local useColorBlock = false

    if IsValid( value.ply ) then
        if TableRound and TableRound().Name == "Homicide" and value.ply:Alive() then
            useColorBlock = true
        end
    ...
    -- Create avatar image or colored block
    if not value.isBot and ctx.panel.displayAvatars then
        if useColorBlock then
            -- Create colored block instead of avatar in Homicide mode
            lines[#lines + 1] = Create.Element( "span", "elColorBlock" )
            Append( lines, "elColorBlock.className = 'avatar';" )
            Append( lines, "elColorBlock.style.display = 'inline-block';" )
            Append( lines, "elColorBlock.style.width = '1.2em';" )
            Append( lines, "elColorBlock.style.height = '1.2em';" )
            Append( lines, "elColorBlock.style.marginRight = '0.2em';" )
            Append( lines, "elColorBlock.style.verticalAlign = 'text-bottom';" )
            Append( lines, "elColorBlock.style.borderRadius = '25%%';" )
            Append( lines, "elColorBlock.style.border = 'solid 1px #555555';" )

            if colors[1] then
                Append( lines, "elColorBlock.style.backgroundColor = '%s';", ColorToRGB( colors[1] ) )
            else
                Append( lines, "elColorBlock.style.backgroundColor = '#888888';" )
            end
        else
            -- Use regular avatar
