...
function SWEP:Reload()
	...
	if self.HoldType == "revolver" then
		...
		timer.Simple(.8, function() self:SetHoldType(self.HoldType) end)
	end

	timer.Simple(.1, function() ply:SetAnimation(PLAYER_RELOAD) end)
	...
	if SERVER then self:ReloadSound() end
end
...
function SWEP:PrimaryAttack()
	...
	if not self.Sightded and not ply:GetNWBool("Suiciding") then
...
function easedLerpAng1(fraction, from, to)
	return LerpAngle(math.ease.OutCubic(fraction), from, to)
end
...
function SWEP:HUDShouldDraw(hud)
	if hud == "CHudWeaponSelection" then return not self:GetNWBool("Sighted") end
end
...
function SWEP:Step()
	...
	-- Visual recoil 
	if self:GetNWFloat("VisualRecoil") > 0 then self:SetNWFloat("VisualRecoil", Lerp(0.075, self:GetNWFloat("VisualRecoil") or 0, 0)) end
	if CLIENT and ply ~= LocalPlayer() then
