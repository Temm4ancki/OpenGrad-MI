			...
			if Tr.Hit then
				if self:CanPickup(Tr.Entity) then
					...
					if self:GetOwner():KeyDown(IN_ATTACK2) then
						surface.SetTexture(surface.GetTextureID("vgui/hud/gmod_closedhand"))
					else
						surface.SetTexture(surface.GetTextureID("vgui/hud/gmod_hand"))
					end
...
function SWEP.GetPlayerStrength(ply)
	...
	return 1 + (math.max(PlyHealth - PlyMaxHealth, 0) ^ 1.2 / (PlyMaxHealth))
end
...
SWEP.Contact = ""
SWEP.Purpose = ""
SWEP.Instructions = " Ваши руки, ЛКМ/Перезарядка: поднять/опустить кулаки;\n В поднятом состоянии: ЛКМ - удар, ПКМ - блок;\n В опущенном состоянии: ПКМ - поднять предмет \n При удержании предмета: E - крутить предмет в воздухе. \n При удержании тела: R - проверить пульс, E - открыть инвентарь"
SWEP.Spawnable = true
...
net.Receive("huyvalues",function(len) -- net receive данные для снятия наручников с игрока
...
function SWEP:SecondaryAttack()
	...
	if SERVER then
		...
		elseif IsValid(tr.Entity) and tr.Entity:IsPlayer() then
			...
			if Dist < self.ReachDistance then -- толкание игроков
				sound.Play("Flesh.ImpactSoft", self:GetOwner():GetShootPos(), 65, math.random(90, 110))
				self:GetOwner():SetVelocity(self:GetOwner():GetAimVector() * 20)
				tr.Entity:SetVelocity(self:GetOwner():GetAimVector() * 100)
...
function SWEP:ApplyForce()
	...
	if IsValid(phys) then
		...
		if self.CarryEnt:GetClass() == "prop_ragdoll" then
			...
			-- 	draw.DrawText(ply and ("развязать "..ply:Nick()) or "", "TargetID", x, y - 40, color_white, TEXT_ALIGN_CENTER )
