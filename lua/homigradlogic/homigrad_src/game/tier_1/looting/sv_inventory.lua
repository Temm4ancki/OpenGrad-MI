...
hook.Add("PlayerSpawn","InventoryPlayerSpawn",function(lootEnt)
...
hook.Add("Player Think","LootingPlayer",function(ply)
	
	if not ply.fake and ply:Alive() and ply:KeyDown(IN_ATTACK2) and ply:KeyPressed(IN_USE) then
		local tr = {}
...
net.Receive("ply_take_item",function(len,ply)
	...
	--if prekol[wep] and not ply:IsAdmin() then ply:Kick("xd))00") return end это skill issue админа и не требует кика челика
