...
hook.Add("Player Think","FakeControl",function(ply,time) --управление в фейке
	...
	if !ply.Otrub then
		...
		if ply:KeyDown(IN_MOVELEFT) or ply:KeyDown(IN_MOVERIGHT) then
			...
			if IsValid(phys) then
				...
				if isGrabbing then
					...
					ang.z = ang.z + direction * rotateSpeed * deltatime
				end
		...
		-[[if ply:KeyDown(IN_USE) then
			local rag = ply:GetNWEntity("Ragdoll")
			local torso = rag:GetPhysicsObjectNum(1)
			local angs = ply:EyeAngles()
			local lookDir = angs:Forward()
			
			-- Двигаем голову
			local headTargetPos = head:GetPos() + lookDir * 4 + Vector(0,0,10)

			-- А теперь тело
			local torsoTargetPos = torso:GetPos() + lookDir * 2
			
			-- -- Настраиваем параметры в зависимости от оружия в руках
			...
			local headShadowParams = {
				secondstoarrive = 0.05,
				pos = headTargetPos,
				angle = angs,
				maxangulardamp = 10,
				maxspeeddamp = 50,
				maxangular = 20, -- быстро
				maxspeed = 150,
				teleportdistance = 0,
				deltatime = FrameTime(),
			}
			
			local torsoShadowParams = {
				secondstoarrive = 0.2,
				pos = torsoTargetPos,
				angle = Angle(0, angs.y, 0), -- чтобы вбок не уходило
				maxangulardamp = 15,
				maxspeeddamp = 40,
				maxangular = 10, -- быстро
				maxspeed = 80,
				teleportdistance = 0,
				deltatime = FrameTime(),
			}
			...
			head:Wake()
			torso:Wake()
			head:ComputeShadowControl(headShadowParams)
			torso:ComputeShadowControl(torsoShadowParams)
		end]]
		if(ply:KeyDown(IN_USE))then
			local phys = head
			local angs = ply:EyeAngles()
